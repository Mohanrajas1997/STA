DELIMITER $$
CREATE DEFINER=`root`@`%` PROCEDURE `pr_sta_get_insiderbenpostcomparisonfromto`(
  in_comp_gid int,
...
me:begin
  ...
  set v_nsdl_benpost_from = in_benpost_from;
  set v_nsdl_benpost_to = in_benpost_to;
  
  set v_cdsl_benpost_from = in_benpost_from;
  set v_cdsl_benpost_to = in_benpost_to;
...
if in_depository_code = 'N' then
  ...
  (
	  ...
	  (
        ...
        and a.depository_code = in_depository_code
		and a.delete_flag = 'N'
	  ...
	  (
		...
		from sta_trn_tbenpost as a
        inner join sta_mst_tcompinsiders as ci on a.comp_gid = ci.comp_gid 
		and (ci.pan_no = a.holder1_pan or ci.pan_no = ci.holder2_pan or ci.pan_no = a.holder3_pan)
		and ci.status_flag = 'Y' and ci.delete_flag = 'N'
		where a.comp_gid = in_comp_gid
		and a.benpost_date = v_nsdl_benpost_from
        and a.depository_code = in_depository_code
