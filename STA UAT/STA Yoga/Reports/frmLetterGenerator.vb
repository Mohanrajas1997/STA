Public Class frmLetterGenerator
    ...
    Private Sub LoadGrid()
        ...
        If cboReportType.Text.ToUpper() = "INWARD DETAIL" Then
            ...
            lsSql &= " a.inward_share_count as 'Inward share count',"
            lsSql &= " a.market_price as 'Market Price',"
            lsSql &= " a.market_value as 'Market Value',"
            lsSql &= " a.threshold_level as 'Threshold Level',"
            lsSql &= " a.tran_code as 'Tran Code',"
            lsSql &= " b.trantype_desc as 'Document Type',"
            lsSql &= " p.docsubtype_desc as 'Document SubType',"
            ...
            lsSql &= " left join sta_mst_tdocsubtype as p on p.docsubtype_code = a.docsubtype_code and p.delete_flag = 'N' "
            lsSql &= " where true "
            lsSql &= lsCond
        ...
        ' Find the index of the "Auto Inward No" column
        Dim autoInwardIndex As Integer = dgvList.Columns("Auto Inward No").Index

        ' Create the "Generate" button column
        Dim btnColumn As New DataGridViewButtonColumn()
        btnColumn.Name = "Generate"
        btnColumn.HeaderText = "Generate"
        btnColumn.Text = "Generate"
        btnColumn.UseColumnTextForButtonValue = True

        ' Insert the "Generate" button column **before** the "Auto Inward No" column
        dgvList.Columns.Insert(autoInwardIndex, btnColumn)
    ...
    Private Sub dgvList_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgvList.CellClick
        ...
        ' Ensure valid row and column index
        If e.RowIndex >= 0 AndAlso e.ColumnIndex >= 0 Then
            ' Check if the clicked column is the "Generate" button column
            If dgvList.Columns(e.ColumnIndex).Name = "Generate" Then
                ' Retrieve inward_no safely (use display name from SQL query)
                Dim inwardno As String = ""
                Dim foliono As String = ""
                Dim inwardid As String = ""
                Dim doctype As String = ""
                Dim shcount As String = ""
                Dim inwdate As String = ""
                Dim tofoliono As String = ""
                Dim companyname As String = ""


                ' Since SQL query uses "Inward No", use that name here
                If dgvList.Rows(e.RowIndex).Cells("Inward No").Value IsNot Nothing Then
                    inwardno = dgvList.Rows(e.RowIndex).Cells("Inward No").Value.ToString()
                End If
                If dgvList.Rows(e.RowIndex).Cells("Folio No").Value IsNot Nothing Then
                    foliono = dgvList.Rows(e.RowIndex).Cells("Folio No").Value.ToString()
                End If
                If dgvList.Rows(e.RowIndex).Cells("Inward Id").Value IsNot Nothing Then
                    inwardid = dgvList.Rows(e.RowIndex).Cells("Inward Id").Value.ToString()
                End If
                If dgvList.Rows(e.RowIndex).Cells("Document Type").Value IsNot Nothing Then
                    doctype = dgvList.Rows(e.RowIndex).Cells("Document Type").Value.ToString()
                End If
                If dgvList.Rows(e.RowIndex).Cells("Inward share count").Value IsNot Nothing Then
                    shcount = dgvList.Rows(e.RowIndex).Cells("Inward share count").Value.ToString()
                End If
                If dgvList.Rows(e.RowIndex).Cells("Received Date").Value IsNot Nothing Then
                    inwdate = dgvList.Rows(e.RowIndex).Cells("Received Date").Value.ToString()
                End If
                If dgvList.Rows(e.RowIndex).Cells("To Folio No").Value IsNot Nothing Then
                    tofoliono = dgvList.Rows(e.RowIndex).Cells("To Folio No").Value.ToString()
                End If
                If dgvList.Rows(e.RowIndex).Cells("Company").Value IsNot Nothing Then
                    companyname = dgvList.Rows(e.RowIndex).Cells("Company").Value.ToString()
                End If

                ' Open the new form and pass inwardno
                Dim newForm As New frmPrintLetter(inwardno, foliono, inwardid, doctype, shcount, inwdate, tofoliono, companyname)
                newForm.Show()
            End If
        End If
    End Sub
    ...
    Private Sub dgvList_CellDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgvList.CellDoubleClick
        ...
        ' Check if the clicked column is "Inward No"
        If dgvList.Columns(e.ColumnIndex).Name = "Inward No" Then
            ' Open frmCoveringLetterDetails
            Dim lnInwardId As Long = 0
            Dim lnInwardNo As String = ""
            Dim lnFolioNo As String = ""

            ' Retrieve values safely
            If dgvList.Rows(e.RowIndex).Cells("Inward Id").Value IsNot Nothing AndAlso Not IsDBNull(dgvList.Rows(e.RowIndex).Cells("Inward Id").Value) Then
                lnInwardId = Convert.ToInt64(dgvList.Rows(e.RowIndex).Cells("Inward Id").Value)
            End If
            If dgvList.Rows(e.RowIndex).Cells("Inward No").Value IsNot Nothing Then
                lnInwardNo = dgvList.Rows(e.RowIndex).Cells("Inward No").Value.ToString()
            End If
            If dgvList.Rows(e.RowIndex).Cells("Folio No").Value IsNot Nothing Then
                lnFolioNo = dgvList.Rows(e.RowIndex).Cells("Folio No").Value.ToString()
            End If

            Dim objFrm As New frmCoveringLetterDetails(lnInwardId, lnInwardNo, lnFolioNo)
            objFrm.ShowDialog()
        Else
            ' Open frmDocHistory for any other column
            Dim lnInwardId As Long = 0
            ...
            Dim objFrm As New frmDocHistory(lnInwardId)
            objFrm.ShowDialog()
        End If
    End Sub
