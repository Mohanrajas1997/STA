DELIMITER $$
CREATE DEFINER=`root`@`%` FUNCTION `fn_get_inwardcompleted`(in_tran_code varchar(16),in_docsubtype_code varchar(16),
in_period_from date,in_period_to date) RETURNS int(11)
BEGIN
declare v_closure_count int(11) default 0;

if (in_tran_code = 'DM' || in_tran_code = 'LS' || in_tran_code = 'IS') then
	select 
		count(a.tran_code) into v_closure_count
	...
	and a.received_date <= in_period_to
	and a.inward_status = 4
...
elseif ((in_tran_code = 'TM') and (in_docsubtype_code = 'TM' or in_docsubtype_code = 'ND'
	...
	select 
		count(b.docsubtype_code) into v_closure_count
	from sta.sta_trn_tinward as a	
...
and a.received_date >= in_period_from 
and a.received_date <= in_period_to
and (a.inward_status = 4 or a.inward_status = 64);
end if; 

set v_closure_count = ifnull(v_closure_count,0);

RETURN v_closure_count;
END$$
DELIMITER ;
