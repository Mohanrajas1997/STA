DELIMITER $$
CREATE DEFINER=`root`@`%` PROCEDURE `pr_sta_bulk_tinward_tm`(
  in in_file_gid int,
  ...
  in in_newfolio_no varchar(32),
  in in_cert_no int(11),
...
me:BEGIN
  ...
  declare v_newfolio_gid int default 0;
...
if err_flag = true then
  ...
  if in_action = 'INSERT' then
	...
	if in_tran_code = 'TM' then
     select
			  group_concat(c.cert_gid),
			  sum(c.share_count) into v_cert_gid,v_share_count
		from sta_trn_tcert as c
		left join sta_mst_tstatus as s on s.status_value = c.cert_status 
        and s.status_type = 'Certificate' and s.delete_flag = 'N'
		where c.folio_gid = v_folio_gid
		and c.cert_status <> v_cert_inactive_status
		and c.delete_flag = 'N'
		order by c.cert_no;
   end if;
    ...
    if err_flag = false then
      ...
      if in_tran_code = 'TM' then
        call pr_sta_ins_queue(v_inward_gid,in_action_by,'N','M','',@queue_gid,@out_result,@out_msg);
		
        select v_inward_gid,v_cert_gid,v_folio_gid,v_newfolio_gid;
        call pr_sta_set_certentrytransmission(v_inward_gid,concat(v_cert_gid,","),v_folio_gid,v_newfolio_gid,31,0,'Bulk',1,'system',@out_result,@out_msg);
        -- call pr_sta_set_queuemove(v_inward_gid,'Success',1,'system',@out_result,@out_msg);
		set out_result = 1;
