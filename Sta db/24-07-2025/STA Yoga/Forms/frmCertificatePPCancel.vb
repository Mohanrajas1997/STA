...
Public Class frmCertificatePPCancel
...
#End Region
    ...
    Private Sub frmCertificatePPCancel_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Dim lsSql As String
        ' company
        lsSql = ""
        ...
        Try
            ...
            With fExcelDatatable
                ...
                For i = 0 To .Rows.Count - 1
                    lsCancelled = Mid(QuoteFilter(.Rows(i).Item("CANCELLED").ToString), 1, 16).ToUpper
                    If lsCancelled = "YES" Or lsCancelled = "NO" Then
                        lsCancelled = lsCancelled
                    Else
                        MsgBox("Cancelled Should be Yes,No. line no:" & (i + 1).ToString, MsgBoxStyle.Critical, gsProjectName)
                        Exit Function
                    End If
                Next
            End With
            ...
            fsSql &= " and file_type = " & gnCertificatePPCancel & " "
            ...
            Using cmd As New MySqlCommand("pr_sta_ins_file", gOdbcConn)
                ...
                cmd.Parameters.AddWithValue("?in_file_type", gnCertificatePPCancel)
            ...
            With fExcelDatatable
                ...
                While i <= .Rows.Count - 1
                    With .Rows(i)
                        ...
                        lsCancelled = Mid(QuoteFilter(.Item("CANCELLED").ToString), 1, 16)
                        lsRemarks = Mid(QuoteFilter(.Item("REMARKS").ToString), 1, 255)

                        Call ConOpenOdbc(ServerDetails)
                        Using cmd = New MySqlCommand("pr_sta_ins_tcertificatepp_cancel", gOdbcConn)
                            cmd.CommandType = CommandType.StoredProcedure
                            ...
                            cmd.Parameters.AddWithValue("?In_cancelled", lsCancelled)
