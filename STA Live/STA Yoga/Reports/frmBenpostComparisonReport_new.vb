...
Public Class frmBenpostComparisonReport_new
    ...
    Private Sub LoadData()
        ...
        If chkFromTo.Checked And lsDepository <> "A" Then
            cmd = New MySqlCommand("pr_sta_get_benpostcomparisonfromto_new1", gOdbcConn)
            cmd.CommandType = CommandType.StoredProcedure
            cmd.Parameters.AddWithValue("?in_comp_gid", lnCompId)
            ...
            If chkboxInsiderList.Checked Then
                cmd1 = New MySqlCommand("pr_sta_get_insiderbenpostcomparisonfromto", gOdbcConn)
                cmd1.CommandType = CommandType.StoredProcedure
                cmd1.Parameters.AddWithValue("?in_comp_gid", lnCompId)
                cmd1.Parameters.AddWithValue("?in_benpost_from", CDate(lsBenpostFrom))
                cmd1.Parameters.AddWithValue("?in_benpost_to", CDate(lsBenpostTo))
                cmd1.Parameters.AddWithValue("?in_depository_code", lsDepository)
            End If
        ElseIf lsDepository <> "A" Then
            cmd = New MySqlCommand("pr_sta_get_benpostcomparison_nsdlcdsl_new1", gOdbcConn)
            cmd.CommandType = CommandType.StoredProcedure
            cmd.Parameters.AddWithValue("?in_comp_gid", lnCompId)
            cmd.Parameters.AddWithValue("?in_benpost_date", CDate(lsBenpostDate))
            cmd.Parameters.AddWithValue("?in_depository_code", lsDepository)
            If chkboxInsiderList.Checked Then
                cmd1 = New MySqlCommand("pr_sta_get_insiderbenpostcomparison_nsdlcdsl", gOdbcConn)
                cmd1.CommandType = CommandType.StoredProcedure
                cmd1.Parameters.AddWithValue("?in_comp_gid", lnCompId)
        ...
        If chkFromTo.Checked And lsDepository = "A" Then
            cmd = New MySqlCommand("pr_sta_get_benpostcomparisonfromto_all1", gOdbcConn)
            cmd.CommandType = CommandType.StoredProcedure
            cmd.Parameters.AddWithValue("?in_comp_gid", lnCompId)
            cmd.Parameters.AddWithValue("?in_benpost_from", CDate(lsBenpostFrom))
            cmd.Parameters.AddWithValue("?in_benpost_to", CDate(lsBenpostTo))
            If chkboxInsiderList.Checked Then
                cmd1 = New MySqlCommand("pr_sta_get_insiderbenpostcomparisonfromto_all", gOdbcConn)
                cmd1.CommandType = CommandType.StoredProcedure
                cmd1.Parameters.AddWithValue("?in_comp_gid", lnCompId)
                cmd1.Parameters.AddWithValue("?in_benpost_from", CDate(lsBenpostFrom))
                cmd1.Parameters.AddWithValue("?in_benpost_to", CDate(lsBenpostTo))
            End If
        ElseIf lsDepository = "A" Then
            cmd = New MySqlCommand("pr_sta_get_benpostcomparison_nsdlcdsl_all1", gOdbcConn)
            cmd.CommandType = CommandType.StoredProcedure
            cmd.Parameters.AddWithValue("?in_comp_gid", lnCompId)
            cmd.Parameters.AddWithValue("?in_benpost_date", CDate(lsBenpostDate))
            If chkboxInsiderList.Checked Then
                cmd1 = New MySqlCommand("pr_sta_get_insiderbenpostcomparison_nsdlcdsl_all", gOdbcConn)
                cmd1.CommandType = CommandType.StoredProcedure
                cmd1.Parameters.AddWithValue("?in_comp_gid", lnCompId)
                cmd1.Parameters.AddWithValue("?in_benpost_date", CDate(lsBenpostDate))
            End If
    ...
    Private Sub btnExport_Click(sender As Object, e As EventArgs) Handles btnExport.Click
        Try
            ...
            If chkboxInsiderList.Checked Then
                Dim grids As New Dictionary(Of String, DataGridView)

                grids.Add("Comparison_RPT", dgvList)
                grids.Add("Insider_Comparison_RPT", dgvList1)

                ExportMultipleGridsToExcel(grids, gsReportPath & "\Report.xls")
            Else
    ...
    Private Function GetNextFriday(d As DateTime) As DateTime
        Dim diff = (DayOfWeek.Friday - d.DayOfWeek + 7) Mod 7
        Return d.AddDays(diff)
    End Function

    Private Sub dtpFrom_ValueChanged(sender As Object, e As EventArgs) Handles dtpFrom.ValueChanged
        If dtpFrom.Value.DayOfWeek <> DayOfWeek.Friday Then
            dtpFrom.Value = GetNextFriday(dtpFrom.Value)
        End If
    End Sub

    Private Sub dtpTo_ValueChanged(sender As Object, e As EventArgs) Handles dtpTo.ValueChanged
        If dtpTo.Value.DayOfWeek <> DayOfWeek.Friday Then
            dtpTo.Value = GetNextFriday(dtpTo.Value)
        End If
    End Sub
End Class