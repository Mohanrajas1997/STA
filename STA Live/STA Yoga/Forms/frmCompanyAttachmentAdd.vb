...
Public Class frmCompanyAttachmentAdd
    ...
    Private Sub btnAdd_Click(sender As Object, e As EventArgs) Handles btnAdd.Click
        ...
        Try
            ...
            Using cmd As New MySqlCommand("pr_sta_trn_tcompanyattachment", gOdbcConn)
                cmd.CommandType = CommandType.StoredProcedure
                cmd.Parameters.AddWithValue("?in_comp_attachment_gid", 0)
                cmd.Parameters("?in_comp_attachment_gid").Direction = ParameterDirection.InputOutput
                cmd.Parameters.AddWithValue("?in_comp_gid", mnCompId)
                cmd.Parameters.AddWithValue("?in_comp_attachmenttype_gid", lnAttachmentTypeId)
                ...
                If lnResult = 1 Then
                    lnAttachmentId = Val(cmd.Parameters("?in_comp_attachment_gid").Value.ToString())
                    If Directory.Exists(gsCompAttachmentPath) = False Then Directory.CreateDirectory(gsCompAttachmentPath)
                    lsDestFile = gsCompAttachmentPath & "\" & lnAttachmentId.ToString & ".sta"
    ...
    Private Sub LoadGrid()
        ...
        Try
            ...
            lsSql &= " a.comp_attachment_gid,"
            lsSql &= " a.insert_by "
            lsSql &= " from sta_trn_tcompanyattachment as a "
            lsSql &= " left join sta_mst_tattachmenttype as b on b.attachmenttype_gid = a.comp_attachmenttype_gid and b.delete_flag = 'N' "
            lsSql &= " where a.comp_gid = " & mnCompId & " "
            ...
            dgvList.Columns("comp_attachment_gid").Visible = False
    ...
    Public Sub New(CompId As Long, AddOnlyFlag As Boolean)
    ...
    Private Sub dgvList_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgvList.CellContentClick
        ...
        Try
            ...
            lnAttachmentId = Val(dgvList.Rows(e.RowIndex).Cells("comp_attachment_gid").Value.ToString)
            lsFileName = dgvList.Rows(e.RowIndex).Cells("File Name").Value.ToString
            lsSrcFile = gsCompAttachmentPath & "\" & lnAttachmentId.ToString & ".sta"
    ...
    Private Sub DeleteFile(AttachmentId As Long)
        ...
        Try
            Using cmd As New MySqlCommand("pr_sta_trn_tcompanyattachment", gOdbcConn)
                cmd.CommandType = CommandType.StoredProcedure
                cmd.Parameters.AddWithValue("?in_comp_attachment_gid", AttachmentId)
                cmd.Parameters("?in_comp_attachment_gid").Direction = ParameterDirection.InputOutput
                cmd.Parameters.AddWithValue("?in_comp_gid", 0)
                cmd.Parameters.AddWithValue("?in_comp_attachmenttype_gid", 0)
