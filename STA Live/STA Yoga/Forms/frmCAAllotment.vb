            ...
            For Each row As DataGridViewRow In dgvRecord2.Rows
                If Not row.IsNewRow Then
                    ...
                    Using cmd As New MySqlCommand("pr_sta_ins_caentry", gOdbcConn)
                        cmd.CommandType = CommandType.StoredProcedure
                        cmd.Parameters.AddWithValue("?in_inward_gid", mnInwardId)
                        cmd.Parameters.AddWithValue("?in_caentry_gid", 0)
                        cmd.Parameters.AddWithValue("?in_caentry_slno", m)
                        cmd.Parameters.AddWithValue("?in_dp_id", row.Cells("dp_id").Value.ToString())
                        cmd.Parameters.AddWithValue("?in_client_id", row.Cells("client_id").Value.ToString())
                        ...
                        cmd.Parameters.AddWithValue("?in_share_price", row.Cells("share_price").Value)
                        cmd.Parameters.AddWithValue("?in_purchase_cost", row.Cells("purchase_cost").Value)
                        cmd.Parameters.AddWithValue("?in_stamp_duty", row.Cells("stamp_duty").Value)
