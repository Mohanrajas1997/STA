...
Public Class frmCategoryChange
...
#End Region
    ...
    Private Sub frmAddressChange_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Call gpBindCombo(lsSql, "category_name", "category_gid", cboNewCategory, gOdbcConn)
    ...
    Private Sub LoadData()
        ...
        cmd = New MySqlCommand("pr_sta_get_folioentrycategory", gOdbcConn)
        ...
        With dt
            If .Rows.Count > 0 Then
                ...
                txtCurrCategory.Text = .Rows(0).Item("curr_category").ToString

                cboNewCategory.Text = .Rows(0).Item("new_category").ToString
    ...
    Private Sub UpdateInformation(ActionStatus As Integer)
        ...
        Try
            If cboNewCategory.Text <> "" And cboNewCategory.SelectedIndex <> -1 Then
                lnCategoryId = Val(cboNewCategory.SelectedValue.ToString())
            End If
            ...
            Using cmd As New MySqlCommand("pr_sta_set_folioentrycategory", gOdbcConn)
                cmd.CommandType = CommandType.StoredProcedure
                cmd.Parameters.AddWithValue("?in_inward_gid", mnInwardId)
                cmd.Parameters.AddWithValue("?in_category_gid", lnCategoryId)
